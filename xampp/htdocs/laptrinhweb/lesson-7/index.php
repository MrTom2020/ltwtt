...
<html>
    ...
    <body>
    <?php session_start();  $_SESSION['dd'] = mt_rand(); ?>
        <?php
...
if (isset($_GET['action']) && $_GET['action'] == "send") {
            ...
            if (!isset($error)) {
                ...
                try {
                    ...
                    //Recipients
                    $mail->setFrom(SMTP_UNAME, "Amazing");
                    $mail->addAddress($_POST['email'], 'Amazing');     // Add a recipient | name is option
                    $mail->addReplyTo(SMTP_UNAME, 'Tên người trả lời');

                    $mail->isHTML(true);                                  // Set email format to HTML
                    $mail->Subject = $_POST['title'];
                    $mail->Body = $_SESSION['dd'];
                    $mail->AltBody = $_SESSION['dd']; //None HTML
                    $result = $mail->send();
                    if (!$result) {
        ...
        <?php } else {
            ...
            <div class="container">
                ...
                <form id="send-email-form" method="POST" action="?action=send">
                    ...
                    <input type="text" name="title" value="Mật khẩu mới" style="visibility: hidden;"/><br/>
